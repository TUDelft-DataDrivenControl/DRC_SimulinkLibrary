function [G,w,Coh,mSuu,mSyy] = spa_avf(u,y,r,dt,Nband,Nfft,ZeroPadding,Wname)
...
if clmode
    ...
    % Squared coherence Cohuy function between u and y - TEMP DISABLE
%     if nu == ny && nargout >= 3
%         Coh = zeros(ny,nu,Nmod);
%         for k = 1:Nmod
%             Coh(:,:,k) = sqrtm(abs(mSyu(:,:,k))*abs(mSyu(:,:,k))/(mSyy(:,:,k)*mSuu(:,:,k)));
%         end
%     end
end
